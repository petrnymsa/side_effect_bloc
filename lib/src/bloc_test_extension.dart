...
import '../side_effect_bloc.dart';

/// FORKED FROM https://github.com/felangel/bloc/blob/def93687228a3ef6893245e991983fdd801a3cea/packages/bloc_test/lib/src/bloc_test.dart

/// Creates a new `bloc`-specific test case with the given [description].
/// [blocWithSideEffectTest] will handle asserting that the `bloc` emits the [expect]ed
/// states (in order) after [act] is executed.
/// [blocWithSideEffectTest] also handles ensuring that no additional states are emitted
...
///
/// [expectSideEffects] is an optional `Function` that returns a `Matcher` which the `bloc`
/// under test is expected to produce side effects after [act] is executed.
///
...
/// ```
///
/// [blocWithSideEffectTest] can optionally be used with a seeded state.
///
/// ```dart
/// blocTest(
...
/// ```
///
/// [blocWithSideEffectTest] can also be used to [skip] any number of emitted states
...
/// ```
///
/// [blocWithSideEffectTest] can also be used to wait for async operations
...
/// ```
///
/// [blocWithSideEffectTest] can also be used to [verify] internal bloc functionality.
///
/// ```dart
/// blocTest(
///   'CounterBloc emits [1] when increment is added',
...
/// ```
///
/// **Note:** when using [blocWithSideEffectTest] with state classes which don't override
...
@isTest
void blocWithSideEffectTest<B extends BlocBase<State>, State, SideEffect>(
  String description, {
    ...
    () async {
      await runZonedGuarded(
        () async {
          ...
          final bloc = build();
          assert(bloc is SideEffectBlocMixin<State, SideEffect>);
          // ignore: invalid_use_of_protected_member, invalid_use_of_visible_for_testing_member
          if (seed != null) bloc.emit(seed());
          final subscription = bloc.stream.skip(skip).listen(states.add);
          final sideEffectsSubscription =
              (bloc as SideEffectBlocMixin<State, SideEffect>)
                  .sideEffects
                  .listen(sideEffects.add);
          try {
            await act?.call(bloc);
          } catch (error) {
          ...
          if (expectSideEffects != null) {
            final dynamic expectedSideEffect = expectSideEffects();
            shallowEquality = '$sideEffects' == '$expectedSideEffect';
            try {
              test.expect(sideEffects, test.wrapMatcher(expectedSideEffect));
            } on test.TestFailure catch (e) {
              if (shallowEquality || expectedSideEffect is! List<SideEffect>)
                rethrow;
              final diff = _diff(expected: expectedSideEffect, actual: states);
              final message = '${e.message}\n$diff';
              // ignore: only_throw_errors
              throw test.TestFailure(message);
            }
          ...
          await sideEffectsSubscription.cancel();
